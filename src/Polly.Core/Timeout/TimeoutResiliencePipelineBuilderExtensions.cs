...
public static class TimeoutResiliencePipelineBuilderExtensions
{
    ...
    /// <summary>
    /// Adds a timeout to the builder with a reloading policy handle for atomic configuration updates.
    /// </summary>
    /// <typeparam name="TBuilder">The builder type.</typeparam>
    /// <param name="builder">The builder instance.</param>
    /// <param name="timeout">The timeout value. This value should be greater than <see cref="TimeSpan.Zero"/>.</param>
    ...
    /// <returns>The same builder instance.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="builder"/> or <paramref name="reloadingHandle"/> is <see langword="null"/>.</exception>
    /// <exception cref="ValidationException">Thrown when the options produced from the arguments are invalid.</exception>
    public static TBuilder AddTimeout<TBuilder>(this TBuilder builder, TimeSpan timeout, TimeoutReloadingPolicyHandle reloadingHandle)
        where TBuilder : ResiliencePipelineBuilderBase
        => builder.AddTimeout(new TimeoutStrategyOptions
        {
            Timeout = timeout
    ...
    [UnconditionalSuppressMessage(
        ...
        "IL2026:Members annotated with 'RequiresUnreferencedCodeAttribute' require dynamic access otherwise can break functionality when trimming application code",
        Justification = "All options members preserved.")]
    public static TBuilder AddTimeout<TBuilder>(this TBuilder builder, TimeoutStrategyOptions options, TimeoutReloadingPolicyHandle reloadingHandle)
        where TBuilder : ResiliencePipelineBuilderBase
    {
        Guard.NotNull(builder);
        Guard.NotNull(options);
        ...
        builder.AddStrategy(context => new TimeoutResilienceStrategy(options, context.TimeProvider, context.Telemetry, reloadingHandle), options);
        return builder;
    }
}
